                ...
                userInventoryArray.map(
                    inventoryObject => {
                        ...
                        return <p
                            ...
                            key={`inventoryItem--${inventoryObject.id}`}>{inventoryObject.inventory.type?.nameOfType}; {inventoryObject.inventory.name} checked out at {hours}:{minutes}:{seconds} on {month}/{date}/{year} {localStorage.getItem("inventory__admin") ? `by ${inventoryObject.user.name}` : null}
